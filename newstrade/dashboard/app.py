...
def _render_symbol_card(row: pd.Series, max_abs_change: float) -> None:
    pct_background, pct_border, pct_text = _pct_change_style(row.get("pct_change"), max_abs_change)
    ...
    <div class="symbol-card">
        ...
        <div class="metric-row">
            ...
            <div class="metric-value pct-pill" style="background:{pct_background};border-color:{pct_border};color:{pct_text};">
                {html.escape(_format_pct(row.get("pct_change")))}
            </div>
        </div>
        <div class="metric-row">
            <div class="metric-label">Previous Close</div>
            <div class="metric-value">{html.escape(_format_decimal(row.get("previous_close_price"), 2))}</div>
        </div>
        <div class="metric-row">
...
def main() -> None:
    ...
    max_abs_change = float(pd.to_numeric(filtered["pct_change"], errors="coerce").abs().fillna(0).max())
